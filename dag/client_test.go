...
func TestRunInsertsHeaderTasksAndDepsAtomically(t *testing.T) {
	...
	view := getDAG(t, f, res.ID)
	is.Equal("atomic", view.Name)
	is.Equal(driver.DAGRunning, view.State)
...
func TestTerminalWorkflowFreesTheIdempotencyKey(t *testing.T) {
	...
	is.Eventually(func() bool {
		return dagState(t, f, first.ID) == driver.DAGSucceeded
...
// txWorkflowFake adds a trivial driver.TxDAGStore[struct{}] over the fake
// so the positive TxRunner path runs without a real transactional backend.
type txWorkflowFake struct {
	*drivertest.Fake
}

func (f *txWorkflowFake) CreateDAGTx(ctx context.Context, _ struct{}, p driver.DAGParams) (bool, uuid.UUID, error) {
	return f.CreateDAG(ctx, p)
}

var _ driver.TxDAGStore[struct{}] = (*txWorkflowFake)(nil)

func TestTxRunnerRequiresTxDAGStore(t *testing.T) {
	t.Parallel()
	is := require.New(t)
