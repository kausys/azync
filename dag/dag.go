...
func New(core *azync.Core, opts ...Option) (*Runtime, error) {
	if core == nil {
		return nil, errors.New("dag: New core is nil")
	}
...
func newRuntime(core *azync.Core, opts []Option, forOpen bool) (*Runtime, error) {
	...
	store, ok := core.Store().(driver.DAGStore)
	if !ok {
		return nil, fmt.Errorf("dag: driver %T does not support dags", core.Store())
	}
...
func (r *Runtime) Close(ctx context.Context) error {
	if r.ownedCore {
		if err := r.worker.Wait(ctx); err != nil {
			r.core.Logger().Warn("dag: Close: worker did not finish draining before ctx ended, closing store anyway", "error", err)
		}
...
type config struct {
	...
	// schedulerTick and dagVacuumInterval shrink the scheduler cadences in
	...
	dagVacuumInterval time.Duration
...
func resolveConfig(defaults azync.Defaults, opts []Option, forOpen bool) (config, error) {
	c := config{
		...
		dagRetention:      defaultRetention,
		schedulerTick:     defaultSchedulerTick,
		dagVacuumInterval: defaultDAGVacuum,
	}
	...
	if !forOpen && len(c.coreOptions) > 0 {
		return config{}, errors.New(
			"dag: WithCoreOptions applies only to Open; New composes over an already-built Core")
	}
	return c, nil
}
...
// WithRetention overrides how long terminal dags (succeeded,
...
func WithRetention(d time.Duration) Option {
	return nonNegativeDuration("WithRetention", d, func(c *config) { c.dagRetention = d })
}
...
func positiveDuration(name string, d time.Duration, set func(*config)) Option {
	return func(c *config) error {
		if d <= 0 {
			return fmt.Errorf("dag: %s: duration must be positive, got %v", name, d)
		}
		set(c)
		return nil
	}
...
func positiveInt(name string, n int, set func(*config)) Option {
	return func(c *config) error {
		if n <= 0 {
			return fmt.Errorf("dag: %s: value must be positive, got %d", name, n)
		}
		set(c)
		return nil
	}
...
func nonNegativeDuration(name string, d time.Duration, set func(*config)) Option {
	return func(c *config) error {
		if d < 0 {
			return fmt.Errorf("dag: %s: duration must not be negative, got %v", name, d)
		}
		set(c)
		return nil
	}
