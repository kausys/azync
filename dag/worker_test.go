...
func (s *recordingStore) ApplyFailurePolicy(ctx context.Context) ([]driver.DAGFailure, error) {
	s.record("policy")
	return s.Fake.ApplyFailurePolicy(ctx)
}

func (s *recordingStore) CompleteDAGs(ctx context.Context) (int64, error) {
	s.record("complete")
	return s.Fake.CompleteDAGs(ctx)
}
...
func TestSchedulerPassRunsInContractOrder(t *testing.T) {
	...
	for i, call := range calls {
		is.Equal(want[i%4], call,
			"scheduler order must be PromoteUnblocked -> CompleteDueSleeps -> ApplyFailurePolicy -> CompleteDAGs (call %d of %v)", i, calls)
	}
}
...
func TestNotReadyReChecksWithoutConsumingBudget(t *testing.T) {
	...
	attempts, err := f.JobAttempts(context.Background(), driver.SourceDAG, parked.ID)
	is.NoError(err)
	...
	attempts, err = f.JobAttempts(context.Background(), driver.SourceDAG, done.ID)
	is.NoError(err)
	is.Empty(attempts)
}
...
func TestBarrierPatternStartsDownstreamExactlyOnce(t *testing.T) {
	...
	// Hold the downstream task open until every upstream has called Run, so
	// live-idempotency still applies: a terminal downstream frees the key and
	// would let late callers insert again (correct store semantics, but not
	// what this barrier race is asserting).
	var releaseDown sync.WaitGroup
	releaseDown.Add(1)
	...
	releaseDown.Done()
	is.Eventually(func() bool {
		return dagState(t, f, page.Items[0].ID) == driver.DAGSucceeded
