// Package dag provides durable static DAGs over an azync Core: a task graph
// declared up front, executed by ordinary job machinery, with durable timers,
// signals, task results, compensation and a per-DAG failure policy. There is
// no workflow-as-code or replay: the DAG is data, and each task handler is a
// plain function that runs at-least-once. For Go-function workflows with
// deterministic replay, see package workflow.
//
// # Model
//
...
//
// Timers: Sleep parks the DAG branch for a duration, durably — no worker
...
//
// Compensation: a task may declare Compensate(args). When the DAG compensates,
// one "comp:<key>" task per succeeded task that declared one runs in reverse
// completion order (a saga).
//
// # Failure policy
//
// Each DAG declares at Define time how a dead task (aborted or out of
// retries) is handled. Cancel — the default — cancels the remaining tasks,
// runs the compensation chain and settles the DAG failed. Suspend parks
// the DAG for a manual decision through the Manager: Retry (reset dead
...
// the tolerant branch keeps running — but a DAG that finishes with any
// dead task settles failed, never succeeded.
//
// A DAG moves through running -> succeeded | failed | cancelled, with
// suspended (parked for an operator) and compensating (saga in flight)
// alongside. Terminal DAGs are removed by the vacuum after the configured
// retention (WithRetention, default 30 days; 0 retains forever). A
// succeeded task's row and result live for as long as its DAG does,
...
// that sweep and are only ever removed as part of their own DAG's vacuum,
// so a task parked behind a long Sleep or WaitSignal never loses the result
// ResultOf and CompleteDAGs depend on.
//
// # Execution
//
...
// DAGs: any number of concurrent Run calls with the same (name, key)
