...
func (d *Definition) validate() error {
	if d.name == "" {
		return errors.New("dag: definition has no name")
	}
	if len(d.tasks) == 0 {
		return fmt.Errorf("dag: definition %q declares no tasks", d.name)
	}
	...
	for _, t := range d.tasks {
		for _, dep := range t.after {
			if !keys[dep] {
				return fmt.Errorf("dag: definition %q: task %q depends on unknown key %q", d.name, t.key, dep)
			}
		}
	}
...
func (d *Definition) validateTask(t taskDecl, seen map[string]bool) error {
	switch {
	case t.key == "":
		return fmt.Errorf("dag: definition %q: a task has an empty key", d.name)
	case seen[t.key]:
		return fmt.Errorf("dag: definition %q: duplicate task key %q", d.name, t.key)
	case strings.HasPrefix(t.key, "$"):
		return fmt.Errorf("dag: definition %q: task key %q uses the reserved \"$\" prefix", d.name, t.key)
	case strings.HasPrefix(t.key, driver.TaskKeyCompensationPrefix):
		return fmt.Errorf("dag: definition %q: task key %q uses the reserved %q prefix",
	...
	case driver.KindSleep:
		if t.sleepFor <= 0 {
			return fmt.Errorf("dag: definition %q: sleep %q duration must be positive, got %v",
	...
	default:
		if t.args == nil {
			return fmt.Errorf("dag: definition %q: task %q has nil args", d.name, t.key)
		}
		if strings.HasPrefix(t.args.Kind(), "$") {
			return fmt.Errorf("dag: definition %q: task %q kind %q uses the reserved \"$\" prefix",
				d.name, t.key, t.args.Kind())
		}
	}

	if t.compensate != nil && strings.HasPrefix(t.compensate.Kind(), "$") {
		return fmt.Errorf("dag: definition %q: task %q compensation kind %q uses the reserved \"$\" prefix",
...
func (d *Definition) validateAcyclic() error {
	...
	return fmt.Errorf("dag: definition %q: dependency cycle involving %s",
