...
func TestConfigInheritsCoreDefaults(t *testing.T) {
	...
	is.Equal(defaultRetention, r.cfg.dagRetention)
...
func TestConfigWorkflowOverrideWinsOverCore(t *testing.T) {
	...
	is.Equal(90*24*time.Hour, r.cfg.dagRetention)
...
func TestConfigExplicitZeroRetentionIsPreserved(t *testing.T) {
	...
	is.Zero(r.cfg.dagRetention, "explicit 0 workflow retention retains terminal dags forever")
}
...
func TestConfigRejectsInvalidValues(t *testing.T) {
	...
	for name, opt := range map[string]Option{
		...
		"WithRetention(-1)":          WithRetention(-time.Hour),
	} {
		_, err := New(core, opt)
		is.Error(err, "%s must be rejected", name)
		is.Contains(err.Error(), "dag: ")
	}
}
