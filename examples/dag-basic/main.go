// Command dag-basic shows a durable DAG workflow shaped like a real
...
// fan-out, and a final idempotent barrier that launches a second dag.
//
...
func run() error {
	...
	// and launches the celebrate dag.
	onboard := dag.Define("onboard-business").
		Task("create-account", createAccount{Email: "founder@example.com"},
			dag.Compensate(deleteAccount{Email: "founder@example.com"})).
		Task("verify-status", verifyStatus{}, dag.After("create-account")).
		WaitSignal("await-approval", dag.After("verify-status")).
		Task("provision", provision{}, dag.After("await-approval")).
		Task("notify", notify{}, dag.After("await-approval")).
		Task("finalize", finalize{}, dag.After("provision", "notify"))
	...
	started, err := r.Client().Run(ctx, onboard, dag.WithIdempotencyKey("onboard-"+businessID))
	if err != nil {
...
func registerHandlers(r *dag.Runtime, prov *provider) error {
	...
	if err := dag.Register(w, func(ctx context.Context, _ verifyStatus) (dag.None, error) {
		...
		default:
			...
			return dag.None{}, dag.NotReady(2 * time.Second)
		}
	}); err != nil {
...
func awaitSuccess(ctx context.Context, m *dag.Manager, id uuid.UUID) error {
	...
	for {
		...
		case <-tick.C:
			...
			if err != nil {
				return fmt.Errorf("get dag: %w", err)
			}
			if view == nil {
				continue
			}
			...
			case dag.StateSucceeded:
				return nil
			case dag.StateFailed, dag.StateCancelled:
...
func reportOutcome(ctx context.Context, m *dag.Manager, id uuid.UUID) {
	...
	page, err := m.List(ctx, dag.Filter{Name: "celebrate"}, 0, 10)
	if err != nil {
		log.Printf("list celebrate: %v", err)
		return
	}
	slog.Info("onboarding succeeded", "celebrate_dags", page.Total)
}
...
func logTask(ctx context.Context, msg string, args ...any) {
	slog.Info(msg, append([]any{
		"task", dag.TaskKey(ctx),
		"attempt", dag.Attempt(ctx),
	}, args...)...)
}
