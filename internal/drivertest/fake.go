...
type fakeJob struct {
	...
	// seq is the global creation-order stamp backing DAGTasks ordering.
	seq int64
}
...
// retention ago. DAG-owned (DAGID != nil) and workflow-as-code (RunID != nil)
// jobs are exempt: cleaned up only by VacuumDAGs / VacuumWorkflows.
func (f *Fake) VacuumCompleted(_ context.Context, source driver.Source, retention time.Duration) (int64, error) {
	...
	for id, j := range f.jobs {
		...
		if j.DAGID != uuid.Nil || j.RunID != uuid.Nil {
			continue
		}
		delete(f.jobs, id)
