...
// engine turns it into a settlement: Abort or an exhausted budget dead-letters
// the job; anything else reschedules it after Delay (or the exponential
// backoff when Delay is zero).
type Outcome struct {
	Kind OutcomeKind
	// Delay overrides the exponential backoff for a retry; 0 means Backoff.
	Delay time.Duration
