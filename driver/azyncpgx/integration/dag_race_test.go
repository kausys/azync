...
// raceTask builds one plain dag task for the race fixtures.
func raceTask(key, kind string, maxAttempts int) driver.DAGTask {
	return driver.DAGTask{Key: key, Kind: kind, Payload: json.RawMessage(`{}`), MaxAttempts: maxAttempts}
}

// raceStore casts the harness Store to its workflow capability.
func raceStore(t *testing.T, h *harness) driver.DAGStore {
	t.Helper()
	ws, ok := h.core.Store().(driver.DAGStore)
	require.True(t, ok, "the pgx store implements driver.DAGStore")
	return ws
}
...
func settleTask(ctx context.Context, t *testing.T, h *harness, kind string, dead bool) {
	t.Helper()
	is := require.New(t)
	jobs, err := h.core.Store().DequeueBatch(ctx, driver.SourceDAG, driver.DequeueParams{
...
func beginCompensateTx(ctx context.Context, t *testing.T, pool *pgxpool.Pool, id uuid.UUID, compKey, compKind string) interface {
	...
	is.NoError(err)
	var state string
	is.NoError(tx.QueryRow(ctx, `SELECT state FROM azync_dags WHERE id = $1 FOR UPDATE`, id).Scan(&state))
	is.Equal("running", state)
	_, err = tx.Exec(ctx, `
INSERT INTO azync_jobs (id, source, kind, state, run_at, max_attempts, payload, enqueued_at, dag_id, task_key)
VALUES ($1, 'dag', $2, 'pending', now(), 3, '{}'::jsonb, now(), $3, $4)`,
		uuid.New(), compKind, id, compKey)
	is.NoError(err)
	_, err = tx.Exec(ctx,
		`UPDATE azync_dags SET state = 'compensating', failure_reason = 'dead tasks: c', updated_at = now() WHERE id = $1`, id)
	is.NoError(err)
	return tx
}

// TestCancelDAGLosingTheSettleRaceIsNotFound races CancelDAG against
// a CompleteDAGs-style settle of the same workflow: the settle commits
...
func TestCancelDAGLosingTheSettleRaceIsNotFound(t *testing.T) {
	is := require.New(t)
	h := newHarness(t)
	ws := raceStore(t, h)
	...
	inserted, _, err := ws.CreateDAG(ctx, driver.DAGParams{
		...
		Tasks: []driver.DAGTask{raceTask("a", "race_cn_a", 3)},
	})
	is.NoError(err)
	is.True(inserted)
	...
	// moved to succeeded, exactly as CompleteDAGs does mid-transaction.
	tx, err := pool.Begin(ctx)
	is.NoError(err)
	_, err = tx.Exec(ctx,
		`UPDATE azync_dags SET state = 'succeeded', completed_at = now(), updated_at = now() WHERE id = $1`, id)
	is.NoError(err)

	cancelErr := make(chan error, 1)
	go func() { cancelErr <- ws.CancelDAG(ctx, id) }()
	...
	case <-time.After(10 * time.Second):
		t.Fatal("CancelDAG never returned")
	}
	view, err := ws.GetDAG(ctx, id)
	is.NoError(err)
	is.Equal(driver.DAGSucceeded, view.State, "the settled terminal state must stand")
}

// TestRetryDAGLosingThePolicyRaceResetsOnlyCompensations races
// RetryDAG against the cancel policy's transition on the same workflow:
...
func TestRetryDAGLosingThePolicyRaceResetsOnlyCompensations(t *testing.T) {
	is := require.New(t)
	h := newHarness(t)
	ws := raceStore(t, h)
	pool := newPool(t, h.base, h.schema)
	...
	inserted, _, err := ws.CreateDAG(ctx, driver.DAGParams{
		...
		Tasks: []driver.DAGTask{a, raceTask("c", "race_rt_c", 1)},
	})
	is.NoError(err)
	is.True(inserted)
	...
	go func() { retryErr <- ws.RetryDAG(ctx, id) }()
	...
	case err := <-retryErr:
		is.NoError(err)
	case <-time.After(10 * time.Second):
		t.Fatal("RetryDAG never returned")
	}

	view, err := ws.GetDAG(ctx, id)
	is.NoError(err)
	is.Equal(driver.DAGCompensating, view.State, "the retry must keep the compensating workflow compensating")
...
// pass against a manual CompensateDAG-style transition: the compensate
...
func TestApplyFailurePolicyLosingTheCompensateRaceSkips(t *testing.T) {
	...
	inserted, _, err := ws.CreateDAG(ctx, driver.DAGParams{
		...
		Tasks: []driver.DAGTask{a, raceTask("c", "race_sp_c", 1)},
	})
	is.NoError(err)
	is.True(inserted)
	...
	view, err := ws.GetDAG(ctx, id)
	is.NoError(err)
	is.Equal(driver.DAGCompensating, view.State,
