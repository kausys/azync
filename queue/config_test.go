...
func TestConfigQueueOverrideWinsOverCore(t *testing.T) {
	t.Parallel()
	is := require.New(t)
	core, err := azync.New(drivertest.NewFake(),
		azync.WithLogger(discardLogger()),
	...
	r, err := New(core, WithLeaseTTL(11*time.Second), WithMaxConcurrency(3))
	is.NoError(err)
	is.Equal(11*time.Second, r.cfg.LeaseTTL, "package option > core option")
	is.Equal(3, r.cfg.MaxConcurrency)
}

func TestConfigQueueOnlyDefaults(t *testing.T) {
	t.Parallel()
	is := require.New(t)
	...
	is.True(r.cfg.cronEnabled, "cron defaults to enabled")
	is.Equal(30*time.Second, r.cfg.cronTick)
	is.Equal(5*time.Minute, r.cfg.jobTimeout)
}
...
func TestConfigExplicitZeroRetentionIsPreserved(t *testing.T) {
	...
	// Explicit 0 at the queue layer means "retain forever" and must not be
...
func TestConfigRejectsInvalidValues(t *testing.T) {
	...
	for name, opt := range map[string]Option{
		...
		"WithDefaultJobTimeout(0)":   WithDefaultJobTimeout(0),
		"WithMaxReaps(0)":            WithMaxReaps(0),
		"WithCronTick(0)":            WithCronTick(0),
		...
		is.Contains(err.Error(), "queue: ")
	}
}

func TestConfigWithCronToggle(t *testing.T) {
	t.Parallel()
	is := require.New(t)
	core, err := azync.New(drivertest.NewFake(), azync.WithLogger(discardLogger()))
	is.NoError(err)

	r, err := New(core, WithCron(false))
	is.NoError(err)
	is.False(r.cfg.cronEnabled)
}
...
func TestConfigDefaultMaxRetriesFlowsToProducer(t *testing.T) {
	t.Parallel()
	is := require.New(t)
	f := drivertest.NewFake()
	r := newTestRuntime(t, f, WithDefaultMaxRetries(4))

	res, err := r.Producer().Enqueue(t.Context(), testArgs{Value: "x"})
	is.NoError(err)
	is.Equal(4, getJob(t, f, res.ID).MaxAttempts)
}
