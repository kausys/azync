...
//
// New subscribers receive future events only; Manager.Replay creates separate
// replay deliveries (flagged Replay) without overwriting history. The delivery
// error taxonomy is deliberately minimal: a plain handler error retries with
// the engine backoff, and Permanent dead-letters immediately — there is no
// RetryAfter or Reportable. A handler panic is recovered and settles as an
// ordinary failure, never crashing the worker process. Because the Worker
// registers one engine kind per
// subscribed handler, an event whose subscriber has no live handler is simply
// never leased; the "missing handler" case disappears by design.
//
